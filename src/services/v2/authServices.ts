...
const loginUser = async ({ username, password }: Auth): Promise<any> => {
  ...
  const PuestosXUserDB = 'ApiCRUD.PuestosXUsuarios'
  const userLogin = await SeguridadDB(usuarioDB)
    .leftJoin(PuestosXUserDB, `${PuestosXUserDB}.Usuario`, '=', `${usuarioDB}.Usuario`)
    .select(`${usuarioDB}.Usuario`, 'EstaActivo', 'IDPerfil', 'IDPuestos')
    .where(`${usuarioDB}.Usuario`, username)
    .then(datos => datos)
